...
async function notifyPatientAnalysisComplete(xray, result, language = "en") {
  let recipient = null;
  let name = "";
  ...
  if (xray.patient_id) {
    ...
    if (doctor?.phone) {
      ...
      name = doctor.name ? (language === "ar" ? ` د. ${doctor.name}` : ` Dr. ${doctor.name}`) : "";
    }
  }
  ...
  let message;
  if (language === "ar") {
    message = `أهلاً${name}، اكتمل تحليل الأشعة السينية الخاص بك عبر MediScan AI. ملخص النتيجة: ${summary}.`;
  } else {
    message = `Hello${name}, your MediScan AI X-ray analysis is complete. Result summary: ${summary}.`;
  }
  ...
  try {
    ...
    setImmediate(async () => {
      try {
        ...
        const aiReportData = await generateReport({ predictions: preds, language: requestLanguage });
        await result.update({ ai_report: aiReportData });
        await notifyPatientAnalysisComplete(xray, result, requestLanguage);
      } catch (err) {
...
async function analyzeXray(req, res, next) {
  try {
    ...
    const requestLanguage = req.body.language || "en";
    const result = await processAiAnalysis(xray.image_path, xray.id, requestLanguage);
    ...
    notifyPatientAnalysisComplete(xray, result, requestLanguage).catch((error) => {
